...
Climates <- readRDS("/home/femeunier/Documents/projects/CongoAS/outputs/LUMIP.Climate.RDS")
...
df.all.av <- df.all %>%
  ...
  summarise(MAP = mean(pre,na.rm = TRUE),
...
ggplot(df.bar) +
  ...
  geom_text(data = df.bar.total,
            aes(x = biome_norm, y = Ntot+50, label = lab)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
saveRDS(df.select.all %>%
          ...
          mutate(keep = TRUE),
        "/home/femeunier/Documents/projects/CongoAS/outputs/LUMIP.Map.change.climate.RDS")
