...
ggplot(data = world) +
  geom_tile(data = biome.select %>% filter(!is.na(biome)),
            aes(x = lon, y = lat,
...
#                       "hpc:/data/gent/vo/000/gvo00074/felicien/R/outputs/CMIP6.NPP.RDS",
#                       "./outputs/"))

all.df <- readRDS("./outputs/CMIP6.ET.RDS") %>%
  filter(timing == "end") %>%
  group_by(scenario,var,model,lat,lon,timing) %>%
  summarise(value.m = mean(value.m),
            .groups = "keep")

all.df <- readRDS("./outputs/CMIP6.NPP.RDS") %>%
  group_by(scenario,var,model,lat,lon,timing) %>%
  summarise(value.m = mean(value.m),
            .groups = "keep") %>%
  ungroup() %>%
  dplyr::select(-var) %>%
  left_join(readRDS("./outputs/CMIP6.ET.RDS") %>%
              group_by(scenario,var,model,lat,lon,timing) %>%
              summarise(value.m = mean(value.m),
                        .groups = "keep") %>%
              ungroup() %>%
              dplyr::select(-var) %>%
              rename(ET = value.m),
            by = c("scenario","model","lat","lon","timing")) %>%
  filter(timing == "end") %>%
  mutate(value.m = value.m/ET,
         var = "WUE") %>%
...
for (cmodel in models){
  ...
  all.df.rspld <- bind_rows(list(all.df.rspld,
                                 resample.df.all.col(bigdf = all.df %>% filter(model == cmodel),
                                                     ...
                                                     var.names = "value.m") %>% mutate(model = cmodel)
  ))
}
...
cdf.biome <- all.df.rspld %>%
  mutate(lat = round(100*lat)/100,
         lon = round(100*lon)/100) %>%
  left_join(biome.select %>%
              ungroup() %>%
              dplyr::select(lat,lon,tmp,pre,biome),
            by = c("lat","lon")) %>%
  filter(!is.na(biome))

ggplot() +
  geom_density(data = all.df %>% filter(!is.na(value.m),
                                        value.m != 0.),
               aes(x = value.m, fill = scenario), alpha = 0.2, color = NA) +
  geom_density(data = all.df.rspld %>% filter(!is.na(value.m)),
               aes(x = value.m, color = scenario), alpha = 0.5, fill = NA) +
...
ggplot(data = cdf.biome.wide.filter %>%
  ...
  labs(x = "",
       ...
       fill = "Model") +
  # scale_y_continuous(limits = c(-1,2.5)*1e-8*86400) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
...
for (cmodel in models){
  ...
  cds <- df.grid %>% filter(model == cmodel,
                            !is.na(lnRR_npp.m)) %>%
  ...
  cds2 <- df.grid %>% filter(model == cmodel,
                             !is.na(diff_npp.m)) %>%
    dplyr::select(tas.cat,pr.cat,diff_npp.m)


  rst.large <- rasterFromXYZ(cds)

  rst.large2 <- rasterFromXYZ(cds2)
