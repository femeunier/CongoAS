...
for (cmodel in models){
    ...
    OP.file <- file.path("./outputs/",paste0("BingBin.",cmodel,".timeseries.RDS"))
        ...
        for (cscenario in scenarios){
            ...
            if (all(file.exists(files2read))){
                tmp <- tryCatch(read.and.filter.ncfiles(ncfiles = files2read,
                                                        coord.analysis = continent2coord("Temperate"),
                                                        var = cvar,
                                                        aggr = FALSE) %>%
                                    ungroup() %>%
                                    distinct(),
...
# scp /home/femeunier/Documents/projects/CongoAS/scripts/read.IPSL.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R
